# bicep-whatif-advisor — Azure What-If Output Summarizer

## Overview

`bicep-whatif-advisor` is a Python CLI tool that accepts Azure Bicep/ARM What-If output via stdin, sends it to an LLM for analysis, and renders a human-friendly summary table in the terminal.
...
```bash
az deployment group what-if ... | bicep-whatif-advisor [OPTIONS]
```
...
```bash
az deployment group what-if ... | bicep-whatif-advisor --format json | jq '.resources[] | select(.action == "Delete")'
```
...
```bash
pip install bicep-whatif-advisor[anthropic]    # installs anthropic SDK
pip install bicep-whatif-advisor[azure]        # installs openai SDK
pip install bicep-whatif-advisor[ollama]       # installs requests (likely already present)
pip install bicep-whatif-advisor[all]          # installs everything
```
...
Support an optional `.bicep-whatif-advisor.yaml` in the user's home directory or current directory for defaults:
          ...
          - task: Bash@3
            ...
            inputs:
              ...
              script: |
                ...
                cat $(Build.ArtifactStagingDirectory)/whatif-output.txt | bicep-whatif-advisor \
                  --ci \
                  --diff-ref origin/main \
                  --bicep-dir infra/ \
                  --drift-threshold high \
...
| Config file | `.bicep-whatif-advisor.yaml` for persisted defaults |
...
| GitHub Action wrapper | Published GitHub Action (`uses: yourorg/bicep-whatif-advisor-action@v1`) that wraps installation and execution |
